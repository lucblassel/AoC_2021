...
def main():
    ...
    while len(counts) > 0:
        ...
        for (
            ...
            p1_score,
            p2_score,
        ), count in counts.items():  # iterate through universe categories
            ...
            for (
                roll,
                times,
            ) in ROLL_COUNTS.items():  # iterate through all possible rolls
                ...
                if new_score >= 21:
                    ...
                    if turn % 2 == 0:
                        new_universes[(new_pos, p2_pos, new_score, p2_score)] += (
                            count * times
                        )
                    else:
                        new_universes[(p1_pos, new_pos, p1_score, new_score)] += (
                            count * times
                        )
