...
class HeatMap:
    ...
    def from_file(cls, filename):
        ...
        with open(filename, "r") as file:
            for line in file:
                heatmap.map.append([int(x) for x in line.strip()])
    ...
    def __repr__(self):
        return "\n".join(
            "".join(
                f"({x})" if self.checked[i][j] else f" {x} " for j, x in enumerate(line)
            )
        ...
        for i_curr, line in enumerate(self.map):
            for j_curr, val in enumerate(line):
                if self.checked[i_curr][j_curr] is not None:
                    continue
                smallest = True
                for i_neighb, j_neighb, val_neighb in self.iterate_neighbors(
                    i_curr, j_curr
                ):
                    if val_neighb <= val:
                        smallest = False
